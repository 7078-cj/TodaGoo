...
export const getRequest = async (endpoint, isToken = false) => {
    if (isToken){
        token = await JSON.parse(await AsyncStorage.getItem("token").access)
    }
...
export const postRequest = async (endpoint, data = {}, isToken = false, isForm = false) => {
    if (isToken){
        token = await JSON.parse(await AsyncStorage.getItem("token").access)
    }
...
export const putRequest = async (endpoint, data = {}, isToken = false, isForm = false) => {
    if (isToken){
        token = await JSON.parse(await AsyncStorage.getItem("token").access)
    }
    try {
...
export const patchRequest = async (endpoint, data = {}, isToken = false, isForm = false) => {
    if (isToken){
        token = await JSON.parse(await AsyncStorage.getItem("token").access)
    }
